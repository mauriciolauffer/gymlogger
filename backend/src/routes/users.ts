...
usersRouter.put("/settings", async (c) => {
  ...
  if (preferred_weight_unit !== undefined && preferred_weight_unit !== null) {
    const unit = await c.env.DB.prepare("SELECT code FROM units WHERE code = ? AND type = 'weight'")
  ...
  if (preferred_length_unit !== undefined && preferred_length_unit !== null) {
    const unit = await c.env.DB.prepare("SELECT code FROM units WHERE code = ? AND type = 'length'")
