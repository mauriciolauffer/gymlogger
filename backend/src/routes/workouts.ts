...
export const workoutsRouter = new Hono<Env>()
  ...
  .post("/start", zValidator("json", startWorkoutSchema), async (c) => {
    const user = c.get("user")!;
  ...
  .post("/:id/exercises", zValidator("json", addWorkoutExerciseSchema), async (c) => {
    const user = c.get("user")!;
  ...
  .post("/:id/sets", zValidator("json", addWorkoutSetSchema), async (c) => {
    const user = c.get("user")!;
    const workoutId = c.req.param("id");
    const body = c.req.valid("json");
    const db = getDb(c);
  ...
  .put("/:id/sets/:setId", zValidator("json", updateWorkoutSetSchema), async (c) => {
    const user = c.get("user")!;
    const workoutId = c.req.param("id");
  ...
  .put("/:id/finish", zValidator("json", finishWorkoutSchema), async (c) => {
    const user = c.get("user")!;
    const workoutId = c.req.param("id");
    const body = c.req.valid("json");
    const db = getDb(c);
